...
from oxyio.models.base import Object
...
class IpBlock(Object, db.Model):
    ...
    # This lovely thing means we count the number of pages whenever we query for brands
    # but without additional SQL queries!
    id = db.Column(db.Integer, primary_key=True)

    ip_count = column_property(
        select([func.count(Ip.id)]).where(Ip.ipblock_id == id)
        # This line stops filters on other tables (brand) being pushed into this query,
        # ie this subquery will *only* touch page.
        .correlate_except(Ip)
    )
